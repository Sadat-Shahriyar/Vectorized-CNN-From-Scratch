    ...
    def fit(self, X_train, y_train, X_val, y_val, learning_rate=0.001, epochs=10, batch_size=32):
        ...
        for epoch in range(epochs):
            ...
            if valid_f1 > self.best_valid_f1:
                ...
                print("Saving model")
                self.save_state_dict("./model/best_model.pkl")
        ...
        return train_loss, train_accruacy, validation_loss, validation_accruacy
...
def read_images_from_folder(folder_path, image_resize_shape_for_lenet = (28,28)):
    images = []
...
def main():
    folder = sys.argv[1]
    images, file_names = read_images_from_folder(folder_path=folder)

    model = Model()
    model.load_state_dict('./best_model.pkl')
    ...
    for name in file_names:
        if name.endswith(".png") or name.endswith(".jpg"):
            csv_matrix.append([name, out[i]])
            i += 1

    filename = folder + "/model_prediction.csv"

    df = pd.DataFrame(csv_matrix)
    df.to_csv(filename, index=False, header=["FileName", "Digit"])
